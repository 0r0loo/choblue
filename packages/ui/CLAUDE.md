...
새 컴포넌트를 추가하거나 기존 컴포넌트에 공개 API를 추가할 때, **반드시** 아래 순서를 모두 완료해야 한다.
구현만 하고 스토리/문서를 빠뜨리는 것은 **미완료**로 간주한다.
...
3. **커밋** — 구현 + 스토리를 함께 커밋
4. **문서 작성** — `apps/docs-ui/content/` 에 컴포넌트 문서(MDX) + 데모(`src/demos/`) 작성 → 별도 커밋

> **체크**: 에이전트는 구현 완료 후 "스토리/문서가 추가되었는가?"를 반드시 자체 검증한다.
> 새 export가 추가되었으면 스토리와 문서도 함께 추가해야 한다.